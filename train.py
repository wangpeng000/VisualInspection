...
parser.add_argument('--obj', default='transistor_plus', type=str)
parser.add_argument('--lambda_value', default=0.001, type=float)
...
parser.add_argument('--maps_num', default=1, type=int)
...
def train():
    ...
    with task('Networks'):
        ...
        enc = MyJigsawPositionHierEncoder(64, D).cuda()

        cls_64 = MyJigsawPositionClassifier(64, D).cuda()
        cls_32 = MyJigsawPositionClassifier(32, D).cuda()
    ...
    with task('Datasets'):
        ...
        datasets[f'pos_64'] = MyJigsawPositionDataset(train_x, K=64, repeat=rep)
        datasets[f'pos_32'] = MyJigsawPositionDataset(train_x, K=32, repeat=rep)
    ...
    for i_epoch in range(args.epochs):
        if i_epoch != 0:
            ...
            for d in loader:
                ...
                loss_pos_64 = MyJigsawPositionClassifier.infer(cls_64, enc, d['pos_64'])
                loss_pos_32 = MyJigsawPositionClassifier.infer(cls_32, enc.enc, d['pos_32'])
        ...
        aurocs = eval_encoder_NN_multiK(enc, obj, args.maps_num)
...
def log_result(obj, aurocs):
    ...
    print(
        f'|K64| Det: {det_64:4.1f} Seg: {seg_64:4.1f} |K32| Det: {det_32:4.1f} Seg: {seg_32:4.1f} |mult| Det: {det_sum:4.1f} Seg: {seg_sum:4.1f} |mult| Det: {det_mult:4.1f} Seg: {seg_mult:4.1f} ({obj})')
